...
def display_table(request):
    table_cursor = collection_Instagram.find({'confs': {'$exists': 'true'}}).sort('likes', pymongo.DESCENDING).limit(30)
...
def display_today_text(request):
    today_cursor = collection_Instagram.find_one(sort=[('date', pymongo.DESCENDING)])
    today_string = dumps(today_cursor)
    return HttpResponse(today_string, content_type='application/json')
